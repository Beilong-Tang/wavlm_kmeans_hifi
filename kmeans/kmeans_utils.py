...
def train(
    ...
    ---------
        ...
        ssl_model: Whether to use SSL model,
            if used SSL model, then it will use SSL to extract features
        device: (Must specify if ssl_model is not None)
            CPU or  GPU.
    ...
    with tqdm(
        ...
        for batch in t:
            if ssl_model is None:
                emb = np.load(batch)  # [T,E]
            else:
                ## Extract features using SSL model
                wav, _ = torchaudio.load(batch)
                wav = wav.to(device)
                emb = ssl_model(wav).squeeze(0).cpu().numpy() # [T,E]

            # extract features from the SSL model
            features_list.extend(emb)
            ...
            if (iteration + 1) % checkpoint_interval == 0:
                ...
                checkpoint_path = os.path.join(
                    save_path,
                    f"kmeans-cluster-{model.n_clusters}-{name}.pt",
                )
